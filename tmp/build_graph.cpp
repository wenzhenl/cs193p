...
main(int argc, const char **argv) {
  ...
  try {
    ...
    opt_parse.add_opt("exist", 'e', "filename for the existing graph", false, exist_graph_filename);
    ...
    // update from an existing graph
    if(!exist_graph_filename.empty()) { 

      std::ifstream g_in(exist_graph_filename.c_str());
      if (!g_in)
        throw SMITHLABException("cannot load the existing graph: " + exist_graph_filename);

      RegularNearestNeighborGraph nng;
      g_in >> nng;
      
      assert(nng.get_maximum_degree() == max_degree);
      ...
      for (FeatVecLookup::const_iterator i(featvecs.begin()); 
          i != featvecs.end(); ++i)
        nng.add_vertex_if_new(i->first);
    }

    // build a completely new graph
    else {
